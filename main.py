...
def start(message):
    ...
    Bonus_button = telebot.types.InlineKeyboardButton(text='bonus', callback_data='bonus')
    markup.add(Task_button, About_button, Inventory_button, Store_button, Fights_button, Bonus_button)
...
def callback_inline(call):
    if call.message:
        if call.data == "about":
            ...
            if db[str(call.message.chat.id)]['main_character'] != None:
                ...
                while True:
                    ...
                    while True:
                        ...
                        udar = (card[db[str(call.message.chat.id)]['main_character']]['Attack'] + db[str(call.message.chat.id)]['bonus_attack']) /(card[db[str(call.message.chat.id)]['main_character']]['Speed'] + db[str(call.message.chat.id)]['bonus_speed'])
            ...
            bot.send_message(call.message.chat.id, f"You bonus\nSpeed: +{db[str(call.message.chat.id)]['bonus_speed']}\nAttack: +{db[str(call.message.chat.id)]['bonus_attack']}\nDefense: +{db[str(call.message.chat.id)]['bonus_defense']}", reply_markup=markup)
            ...
            global price
            price = price_list[price_list.index(db[str(call.message.chat.id)][('bonus_' + call.data[6:])] + 1)]
